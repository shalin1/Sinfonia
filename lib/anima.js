...
function run(object) {
  console.log `${object} at ${presstime}`
  if (new Date().getTime() - pressTime > 1000) {
  ...
  switch (object.name) {
    ...
    debugger
      object.scale(1.02)
      if (new Date().getTime() - object.createTime > 2000) {
        item.remove(object)
      }
      break;
    case 'fastTunnel':
...
function tunnel() {
  ...
  circle.createdAt = new Date().getTime();
